<div>
	...
	function loadWork(apiInfo) {
		document.getElementById('modifypage_url').value = apiInfo.Sitemap.URL;
		document.getElementById('modifypage_title').value = apiInfo.Title;
		document.getElementById('modifypage_keywords').value = apiInfo.Sitemap.Keywords;
		document.getElementById('modifypage_description').value = apiInfo.Description;
		document.getElementById('modifypage_status').value = apiInfo.Sitemap.Status;
	}
	
	function getOneWork(url) {
		ajaxAPI('GET','/api/editor/get?site='+url,{}).then(
			([status,response]) => {
				loadWork(response);
				windowChange('admin_config');
			},
			([status,response]) => {
				alert('Error: Fail to read work: ' + status);
			}
		);
	}
	</script>
...
<?php else: ?>
		<a id="a_admin_new" onclick="windowChange(this.id.substr(2))">新建</a>
		<a id="a_admin_config" onclick="windowChange(this.id.substr(2))">设置</a>
		<a id="a_admin_content" onclick="windowChange(this.id.substr(2))">内容</a>
		<a id="a_admin_addon" onclick="windowChange(this.id.substr(2))">附加项</a>
		<a id="a_admin_preview" onclick="windowChange(this.id.substr(2))">预览</a>
		<a id="a_admin_action" onclick="windowChange(this.id.substr(2))">操作</a>
<?php endif; ?>
		<script>
	function windowChange(name) {
		//Reset all and show the selected one
		Array.prototype.slice.call(document.querySelectorAll('#admin_window>a')).map(x => {
			x.style.backgroundColor = '#CCCCCC';
		});
		Array.prototype.slice.call(document.querySelectorAll('#admin_window>[id^=admin_]')).map(x => {
	...
	window.addEventListener('load', () => {windowChange('admin_new')} ); 
		</script>
...
<?php if ($PAGELANG == 'en'): ?>
			<h2>New</h2>
			<div class="info">
				Create a new one here, or select one from the left to edit.<br />
			</div>
			<label>Category</label>
			<select id="createpage_url_prefix">
				<?php
					foreach (explode(',',$TEMPLATEDATA['Category']) as $cate) {
						$cate = trim($cate);
			...
			<div class="info">
				Category: What category should it go?
			</div>
			<label>URL</label>
			<input id="createpage_url" type="text" pattern="[A-Za-z0-9_:\-\.]{2,60}" />
			<div class="info">
				Specific the URL here. You can use alphabetic characters, numerical characters, underscore, score, colon, slash, and dot. 2 to 60 characters. (A-Za-z0-9_-:.)
			</div>
			<label>Action</label>
			<p>The URL will be: <code id="createpage_url_final"></code></p>
			<button type="submit" id="createpage_submit">Create</button>
			<div class="info">
				Click this button to create.<br />
				After successfully create it, you can select it on the left and then edit it.
			</div>
<?php else: ?>
			<h2>新建</h2>
			<div class="info">
			...
			<input id="createpage_url" type="text" pattern="[A-Za-z0-9_:\-\.]{2,60}" />
			<div class="info">
				指定作品的URL，只能使用英文字母/数字/下划线/减号/冒号/斜杠/点，2至60个字符。(A-Za-z0-9_-:.)
			</div>
			<label>操作</label>
			<p>将会使用的URL为：<code id="createpage_url_final"></code></p>
			<button type="submit" id="createpage_submit">创建主题</button>
			<div class="info">
				点击按钮创建作品。<br />
...
<?php endif; ?>
			<script>
	function createNewConfirmURL() {
		url = document.getElementById('createpage_url_prefix').value.trim().toLowerCase() + '/' + document.getElementById('createpage_url').value.trim();
		document.getElementById('createpage_url_final').innerText = url;
		return url;
	}
	document.getElementById('createpage_url_prefix').onchange = createNewConfirmURL;
	document.getElementById('createpage_url').onchange = createNewConfirmURL;
	document.getElementById('createpage_url').onkeyup = createNewConfirmURL;
	
	document.getElementById('admin_new').onsubmit = (e) => {
		e.preventDefault();
		if (
			document.getElementById('createpage_url_prefix').validity.patternMismatch ||
			document.getElementById('createpage_url').validity.patternMismatch
		) return;
		
		var url = createNewConfirmURL();
		ajaxAPI('POST','/api/editor/post?site='+url,{
			"category"	: document.getElementById('createpage_url_prefix').value,
		...
		<div id="admin_upload_files_preview_template" style="display: none;">
...
<script>
	...
	function xUploadRender(file) {
		...
		reader.onload = function(){
			...
			right.innerHTML = document.getElementById('admin_upload_files_preview_template').innerHTML;
